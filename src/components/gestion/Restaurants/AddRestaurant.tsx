...
export default function AddRestaurant({ onCancel, onSuccess, restaurant }: AddRestaurantProps) {
  ...
  const validateForm = () => {
    ...
    if (isNaN(parseFloat(formData.latitude.toString())) || !formData.latitude) {
      errors.latitude = 'Latitude invalide'
    }
    
    if (isNaN(parseFloat(formData.longitude.toString())) || !formData.longitude) {
    ...
    if (!restaurant?.id) {
      if (!formData.managerFullname.trim()) {
        errors.managerFullname = 'Le nom du gérant est requis'
      }
      
      if (!formData.managerEmail.trim() || !/\S+@\S+\.\S+/.test(formData.managerEmail)) {
        errors.managerEmail = 'Email du gérant invalide'
      }
      
      if (!formData.managerPhone.trim()) {
        errors.managerPhone = 'Le téléphone du gérant est requis'
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Si c'est une création (nouveau restaurant)
        ...
        if (formData.imageFile) {
          ...
          try {
            ...
            toast.error(`Erreur: ${error.message || 'Impossible de créer le restaurant'}`);
            setIsLoading(false);
          }
        } else {
      ...
      toast.error(`Erreur: ${error.message || 'Impossible de traiter le restaurant'}`)
    } finally {
      setIsLoading(false)
    }
  }

  // Jours de la semaine pour l'horaire
  const daysOfWeek = [
    { id: '1', label: 'Lundi' },
    { id: '2', label: 'Mardi' },
    { id: '3', label: 'Mercredi' },
  ...
  return (
    <form onSubmit={handleSubmit} className="overflow-y-auto max-h-[82vh] px-4 pb-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <Input
                ...
                className={`w-full h-[42px] rounded-xl bg-white border ${formErrors.address ? 'border-red-500' : 'border-[#D8D8D8]'} pl-10 pr-4 text-[13px] placeholder-gray-400`}
              />
            </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <Input
              ...
              className={`w-full h-[42px] rounded-xl bg-white border ${formErrors.managerFullname ? 'border-red-500' : 'border-[#D8D8D8]'} px-4 text-[13px] placeholder-gray-400`}
            />
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Input
                ...
                className={`w-full h-[42px] rounded-xl bg-white border ${formErrors.managerEmail ? 'border-red-500' : 'border-[#D8D8D8]'} px-4 text-[13px] placeholder-gray-400`}
              />
            ...
            <div>
              ...
              <Input
                ...
                required
                placeholder="+225 XX XX XX XX"
                className={`w-full h-[42px] rounded-xl bg-white border ${formErrors.managerPhone ? 'border-red-500' : 'border-[#D8D8D8]'} px-4 text-[13px] placeholder-gray-400`}
              />
      ...
      {/* Modal pour afficher les identifiants du manager créé */}
