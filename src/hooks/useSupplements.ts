...
export const useSupplements = (initialSupplements: Record<string, any> = {}) => {
   const [supplementOptions, setSupplementOptions] = useState<Record<string, SupplementOption[]>>({
   ...
   const [supplementQuantities, setSupplementQuantities] = useState<Record<string, Record<string, number>>>({
    [SUPPLEMENT_TYPE.ACCESSORY]: {},
    [SUPPLEMENT_TYPE.FOOD]: {},
    [SUPPLEMENT_TYPE.DRINK]: {}
  });
  ...
  const [isLoading, setIsLoading] = useState(false);
   ...
   const initializeSupplements = (supplements: Record<string, any>) => {
    if (!supplements) return;
    
     const extractSupplementData = (items: any[], type: SUPPLEMENT_TYPE) => {
     ...
     Object.values(SUPPLEMENT_TYPE).forEach(type => {
      if (supplements[type]) {
        const { ids, quantities } = extractSupplementData(supplements[type], type as SUPPLEMENT_TYPE);
